...
library WeightedStableMath {
    ...
    error WeightedStableInvariantDidNotConverge();

    /// @notice The iterations to calculate the balance didn't converge.
    error WeightedStableComputeBalanceDidNotConverge();
    ...
    struct StableMathParams {
        ...
        uint256 subNumerator;
        uint256 subDenominator;
    }
    ...
    function computeInvariant(
        ...
        // invariant                                                                                 //
        // D = invariant                                       D^2 Pwo                               //
        // A = amplification coefficient      A S + D = A D + -----------                            //
        // S = sum of balances                                    Pw                                 //
        // Pw = product of weighted balances                                                         //
        // Pwo = product of weights in the power of the weight                                       //
        **********************************************************************************************/

        StableMathParams memory params;

        params.sum = 0; // S in the Curve version
        params.numTokens = balances.length;
        for (uint256 i = 0; i < params.numTokens; ++i) {
            params.sum = params.sum + balances[i];
        }
        if (params.sum == 0) {
            return 0;
        }

        // Calculate the product of weights in the power of the weight.
        params.Pwo = FixedPoint.ONE;
        for (uint256 i = 0; i < params.numTokens; ++i) {
            params.Pwo = params.Pwo.mulDown(weights[i].powDown(weights[i]));
        }

        params.invariant = params.sum; // Constant sum invariant as initial value for Newton method

        for (uint256 i = 0; i < 255; ++i) {
            params.D_P = params.invariant.mulDown(params.Pwo);

            for (uint256 j = 0; j < params.numTokens; ++j) {
                params.D_P = params.D_P.divDown(balances[j].powDown(weights[j]));
            }

            params.prevInvariant = params.invariant;

            params.sumNumerator = (amplificationParameter * params.sum) / AMP_PRECISION + params.invariant;
            params.subNumerator =
                ((amplificationParameter * params.invariant) / AMP_PRECISION) +
                (params.invariant.mulDown(params.D_P));
            params.subDenominator = (FixedPoint.ONE * amplificationParameter) / AMP_PRECISION + (2 * params.D_P);

            if (params.subNumerator > params.sumNumerator) {
                if (params.subDenominator > FixedPoint.ONE) {
                    params.invariant =
                        params.invariant +
                        (params.subNumerator - params.sumNumerator) /
                        (params.subDenominator - FixedPoint.ONE);
                } else {
                    params.invariant =
                        params.invariant -
                        (params.subNumerator - params.sumNumerator) /
                        (FixedPoint.ONE - params.subDenominator);
                }
            } else {
                if (params.subDenominator > FixedPoint.ONE) {
                    params.invariant =
                        params.invariant -
                        (params.sumNumerator - params.subNumerator) /
                        (params.subDenominator - FixedPoint.ONE);
                } else {
                    params.invariant =
                        params.invariant +
            ...
            unchecked {
                ...
                if (params.invariant > params.prevInvariant) {
                    if (params.invariant - params.prevInvariant <= 1) {
                        return params.invariant;
                    }
                } else if (params.prevInvariant - params.invariant <= 1) {
                    return params.invariant;
                }
            }
        }

        revert WeightedStableInvariantDidNotConverge();
    }
    ...
    //     revert WeightedStableComputeBalanceDidNotConverge();
    // }
}
